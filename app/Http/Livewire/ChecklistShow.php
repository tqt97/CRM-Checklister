...
class ChecklistShow extends Component
{
    ...
    public function mount($checklist)
    {
        ...
        // $this->opened_tasks = $checklist->tasks()->whereNull('completed_at')->get();
        ...
        if ($task) {
            ...
            if ($user_task) {
                if (is_null($user_task->completed_at)) {
                    ...
                    $this->emit('task_completed', $task_id, $task->checklist_id);
                } else {
                    $user_task->update(['completed_at' => NULL]);
                    $this->emit('task_completed', $task_id, $task->checklist_id, -1);
                }
            } else {
